...
class ConsoleUI:
    ...
    def convert_type_to_string(self, file_types):
    ...
    def print_summary(self, file_types, sensitivity, count):
        print("-" * 63)
        print(" Summary of your selections:")
        print(f"    - File types selected: {self.convert_type_to_string(file_types)}")
    ...
    def console_ui(self):
        self.print_banner()  # Display the banner
        path = self.get_valid_path()  # Get a valid directory path from the user
        file_types = self.get_file_types()  # Get file types from the user
        sensitivity = self.get_sensitivity_level()  # Get sensitivity level from the user
        count = self.get_number_of_files()  # Get the number of files to generate
        self.print_summary(file_types, sensitivity, count)  # Print a summary of the selections
